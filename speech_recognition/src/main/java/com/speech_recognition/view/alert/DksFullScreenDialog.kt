package com.speech_recognition.view.alert
...
import com.speech_recognition.R
import com.speech_recognition.view.progress.DksProgressView
import com.speech_recognition.view.progress.DksProgressViewListener
...
data class DksLiveObservers(
    val speechResult: MutableLiveData<String>, val speechFrequency: MutableLiveData<Float>,
    val oneStepVerify: MutableLiveData<Boolean>, val resetButtons: MutableLiveData<Boolean>,
    val ejectView: MutableLiveData<Boolean>
)

class DksFullScreenDialog(
    private val dksProgressViewLayout: Int,
    private val manager: FragmentManager,
    private val observers: DksLiveObservers,
    private val listener: DksFullScreenDialogListener
) : DialogFragment() {
    companion object {
    ...
    override fun onCreateView(
        ...
        if (dialogView is DksProgressView) {
            val defaultProgressTxt =
                dialogView.findViewById<TextView>(R.id.tv_progress_message).text.toString()
            dialogView.setProgressViewListener(object : DksProgressViewListener {
                override fun onClickedPositive() {
                    listener.onSuccess(dialogView.findViewById<TextView>(R.id.tv_progress_message).text.toString())
                }
            ...
            PERMISSION_REQUEST_CODE -> {
                grantResults.forEach {
                    if (it != PackageManager.PERMISSION_GRANTED) {
                        ...
                        listener.onMicrophonePermissionStatus(
                            statusGiven = false,
                            restartSpeechOps = false
                        )
                        return
                    }
                }
